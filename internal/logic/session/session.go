...
const (
	sessionKeyAdminInfo    = "SessionKeyAdminInfo" // 用户信息存放在Session中的Key
...
func (s *sSession) SetUser(ctx context.Context, user *entity.AdminInfo) error {
	return service.BizCtx().Get(ctx).Session.Set(sessionKeyAdminInfo, user)
}
...
func (s *sSession) GetUser(ctx context.Context) *entity.AdminInfo {
	...
	if customCtx != nil {
		v, _ := customCtx.Session.Get(sessionKeyAdminInfo)
		if !v.IsNil() {
...
func (s *sSession) RemoveUser(ctx context.Context) error {
	customCtx := service.BizCtx().Get(ctx)
	if customCtx != nil {
		return customCtx.Session.Remove(sessionKeyAdminInfo)
	}
	return nil
}
